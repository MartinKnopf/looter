...
(function() {
  ...
  clickStartGame.subscribe(function() {
    var balance, canBuy, canSell, cannotBuy, cannotSell, clickBuy, clickNextTurn, clickSell, gameOver, lastTurn, maxPrice, minPrice, newBuy, newPrice, newSell, newStockAmount, newTurn, numStocks, oneStock, profit, progressBar, resettableTimer, twoStocks;
    ...
    numStocks = oneStock;
    numStocks.subscribe(function(x) {
    ...
    maxPrice = minPrice.map(function(x) {
      return Math.floor(x * _.random(3, 8));
    });
    newPrice = newTurn.combineLatest(minPrice, maxPrice, function(turn, min, max) {
      return _.random(min, max);
    }).takeUntil(lastTurn).publish();
